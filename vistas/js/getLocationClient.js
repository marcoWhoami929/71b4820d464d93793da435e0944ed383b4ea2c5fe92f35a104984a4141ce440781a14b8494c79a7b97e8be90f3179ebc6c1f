...
function initialize() {
  ...
  autocomplete = new google.maps.places.Autocomplete(
     (document.getElementById('direccionPerfil')),
      { types: ['geocode'] });
...
function fillInAddress() {
  ...
  for (var i = 0; i < place.address_components.length; i++) {
    ...
    $("#coordenadasPerfil").val(coordenadas);
     $("#latRecordatorio").val(place.geometry.location.lat());
    $("#longRecordatorio").val(place.geometry.location.lng());

    mapa.initMap(place.geometry.location.lat(),place.geometry.location.lng());
}
