	...
	$("#btnGenerarNuevoEvento").on("click",function(){
		...
		if (idProspecto === undefined) {
			...
			swal({
				...
				confirmButtonText: 'Si, crear el evento!',
			...
			var datos = new FormData();
			datos.append("idProspectoRecordatorio", '0');
			...
			$.ajax({
				...
				success: function(response) {
					...
					swal({
						type: 'warning',
						html: 'El evento no pudo ser registrado'
					});
				}
...
$(".btnCargarNuevoProspecto").on('click', function() {
	...
	var latitud = localStorage.latitud;
	var longitud = localStorage.longitud;
	var tipoClientePerfil = $("#tipoClientePerfilCrear").val();
	...
	var comentariosPerfil = $("#comentariosPerfilCrear").val();
    	...
    	*/
    	if (nombrePerfil === "" || direccionPerfil === "" || latitud === "" || longitud === ""  ) {
    		...
    		datos.append('tipoClientePerfilCrear',tipoClientePerfil);
...
$("#btnEliminarEvento").click(function(){
	...
	var datos = new FormData();
	datos.append("idEventoDelete", idEvento);
	datos.append("eventoDelete", eventoFinal);
	datos.append("tituloEventoDelete", titulo);
	datos.append("prospectoEventoDelete", nombreProspecto);
	datos.append("idProspectoEventoDelete", idProspecto);
	...
	$.ajax({
		...
		success: function(response) {
			...
			if (responseFinal == "ok") {	
				...
				swal("Upss", "No se pudo eliminar el evento.", "info");
			}
